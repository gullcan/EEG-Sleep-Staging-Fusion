...
# 命令行传参
parser = argparse.ArgumentParser(description="Train Manual Feature Sleep Staging Model")
...
#加载 manual feature 数据
files = scan_manual_feature_files(manual_dir)
...
if len(subject_ids) < 3:
    raise RuntimeError(
        ...
        "Generate manual feature cache for more files before real manual training."
    )
...
train_dataset = LazyManualFeatureSequenceDataset(
    files=train_files,
    seq_len=seq_len,
    debug=args.debug
)

validate_dataset = LazyManualFeatureSequenceDataset(
    files=val_files,
    seq_len=seq_len,
    debug=args.debug
)
...
print("\nManual feature sequence counts:")
...
print(f"Check X_manual shape: {X_check.shape}")
...
model_name = "ManualBiLSTM" if bidirectional else "ManualLSTM"
