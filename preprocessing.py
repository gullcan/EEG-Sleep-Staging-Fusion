...
def edf2numpy(signal_file_path, annotation_file_path, select_channels):
    ...
    sampling_rate = X_data.getSampleFrequency(channels.index(select_channels[0]))
    n_epoch_samples = int(sampling_rate * epoch_duration)
    ...
    for select_channel_idx in range(1,len(select_channels)):
        sampling_rate = X_data.getSampleFrequency(channels.index(select_channels[select_channel_idx]))
        n_epoch_samples = int(sampling_rate * epoch_duration)        
