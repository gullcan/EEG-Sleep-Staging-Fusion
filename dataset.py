...
class EdfDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, seq_len, is_train=True, shuffle_seed=42):
        ...
        files = sorted(files)
        files = files[:2]  # Geçici: sadece ilk .npz dosyasını kullan
        print("Toplam kullanılan dosya sayısı:", len(files))
        # 随机抽选训练集和测试集样本
        ...
        for fi in nfiles:
            data = np.load(fi)
            # GEÇİCİ TEST:
            # Bellek dolmasın diye sadece ilk 5 sequence'ı kullanıyoruz.
            #for seq_idx in range(len(data['x'])//seq_len):
            for seq_idx in range(min(5, len(data['x'])//seq_len)):
...
if __name__ == "__main__":
    ...
    # GEÇİCİ TEST:
    #edf_dataloader = torch.utils.data.DataLoader(edf_dataset, batch_size=15, shuffle=True)
    edf_dataloader = torch.utils.data.DataLoader(edf_dataset, batch_size=1, shuffle=True)
