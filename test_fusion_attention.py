...
from network import FusionAttentionSleepNet


labels = ["W", "N1", "N2", "N3", "REM"]


parser = argparse.ArgumentParser(description="Test Raw EEG + Manual Feature Fusion Attention Model")
...
parser.add_argument("--model_path", type=str, default="./models/model_FusionAttentionBiLSTM_seq20_e100.pt")
...
if len(paired_raw_files) == 0:
    raise RuntimeError(
        ...
        "Generate manual feature cache before testing fusion attention model."
    )
...
print(f"Fusion attention subjects found: {len(subject_ids)}")
...
all_attention = torch.cat(all_attention, dim=0).numpy()
...
print(f"attention shape: {all_attention.shape}")
print(f"attention min  : {all_attention.min():.6f}")
print(f"attention max  : {all_attention.max():.6f}")
print(f"attention mean : {all_attention.mean():.6f}")
print(f"attention row sum mean: {all_attention.sum(axis=1).mean():.6f}")


os.makedirs("./figures", exist_ok=True)
os.makedirs("./results", exist_ok=True)

model_name = "FusionAttentionBiLSTM" if args.bidirectional else "FusionAttentionLSTM"
...
# Ortalama attention dağılımı
mean_attention = all_attention.mean(axis=0)

plt.figure(figsize=(10, 4))
plt.plot(np.arange(1, args.seq_len + 1), mean_attention, marker="o")
plt.xlabel("Epoch position in sequence")
plt.ylabel("Mean attention weight")
plt.title(f"Mean Temporal Attention - {model_name}")
plt.xticks(np.arange(1, args.seq_len + 1))
plt.tight_layout()
plt.savefig(
    f"./figures/mean_attention_{model_name}_seq{args.seq_len}_e100.pdf",
    dpi=300,
    bbox_inches="tight"
)
plt.show()
...
# Attention değerlerini kaydet
np.savez(
    f"./results/attention_weights_{model_name}_seq{args.seq_len}_e100.npz",
